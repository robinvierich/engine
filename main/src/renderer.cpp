...
Matrix4 createProjectionMatrix(float fov, float farPlane, float nearPlane)
{
// The default camera looks down the negative z-axis.

// There are a few goals of the projection:
//   1. Map a point from 3d space to a 2d point on the near clipping plane.
//   2. Normalize z values between 0 -> 1 based on near and far clipping plane.
...
Matrix4 cameraToScreenProj = createProjectionMatrix(60, 1, 100);
...
bool initOpenGL(HWND hWnd)
{
    ...
    { // If the OpenGL 4.x context creation extension is available
